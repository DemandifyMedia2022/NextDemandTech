...
export const metadata: Metadata = {
  title: 'Learning Resources | DemandTech',
  description: 'Guides, whitepapers, and resources to help you learn.'
}
...
export default async function LearningResourcesPage() {
  const items: ResourceDoc[] = await client.fetch(allLearningResourcesQuery)

  return (
    <div className="min-h-screen bg-[#F0F1FA]">
      <div className="max-w-6xl mx-auto px-4 py-16">
        <h1 className="text-4xl md:text-6xl font-semibold" style={{ fontFamily: 'Clash Display', color: '#000cf8' }}>Learning Resources</h1>
        <p className="mt-3 text-lg text-black/70" style={{ fontFamily: 'Neue Montreal' }}>Download resources to level up your knowledge.</p>
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {items?.length ? items.map((doc) => {
            ...
            <p className="text-black/60">No resources yet.</p>
          )}
        </div>
      </div>
    </div>
